...
export const markFailed = async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Order marked as failed', data: { order } })
...
export const getAnalytics = async (req, res) => {
    try {
        ...
        const [
            ...
            Order.countDocuments({ status: { $in: ['pending', 'assigned', 'picked_up', 'in_transit'] } }),
        ])
        ...
        res.json({
            ...
            data: {
                ...
                successRate: totalToday > 0
                    ? ((deliveredToday / totalToday) * 100).toFixed(1)
                    : 0,
            }
        })
